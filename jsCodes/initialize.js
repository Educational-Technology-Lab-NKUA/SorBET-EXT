...
const platform=true	//set this to true when on platform, false if not on platform

/*
doesn't work for som reason

const wsXmlGSPath = "https://cdn.jsdelivr.net/gh/hermolaou/sorbet/xml/workspaceGamestart.xml";
const toolboxXmlGSPath = "https://cdn.jsdelivr.net/gh/hermolaou/sorbet/xml/toolboxGamestart.xml";
*/
...
/*
doesn't work for som reason
const wsXmlGPPath = "https://cdn.jsdelivr.net/gh/hermolaou/sorbet/xml/workspaceGameplay.xml";
const toolboxXmlGPPath = "https://cdn.jsdelivr.net/gh/hermolaou/sorbet/xml/toolboxGameplay.xml";
*/
...
var blocklySpeedField, blocklyDensityField
...
$(document).ready(function () {
	...
	//design mode functon
	//Jquery text editor for game instructions
	$("#instructionsEditor").val("Welcome to SORBET! Try to correctly classify as many of the falling objects as possible!");
	$("#instructionsEditor").jqte(
		{placeholder: "You can write some instructions for the player", format: false});
    ...
    */
	...
	loadXml(toolboxXmlGSPath).then((toolboxXml) => {
		...
		/*if ((jsonGs = localStorage.getItem("jsongs"))) {
			Blockly.serialization.workspaces.load(JSON.parse(jsonGs), workspaceGamestart);
			...
			//blocklySpeedField = block.getField("NUM");

			// Get the block where you want to set the numeric field
			block = workspaceGamestart.getBlockById("`G^Yg+5BU6%)0D2ilXo5");

			// Get the numeric field within the block
			blocklyDensityField = block.getField("NUM");
			...
			loadXml(wsXmlGSPath).then((workspaceXml) => {
				...
				blocklySpeedField.setValue(startingValue);
				...
				// Assign the starting value to the field's setValue method
				blocklyDensityField.setValue(30);
				...
				loadXml(toolboxXmlGPPath).then((toolboxXml) => {
					...
					/*if ((jsonGp = localStorage.getItem("jsongp")))
						...
						//alert("Something must be seriously wrong.")

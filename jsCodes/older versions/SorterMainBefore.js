...
function SorterGame(canvas, gameObjects, categories, elementsPerRound) {
	...
	this.sortedObjectsEvent = function () {
		if (!fromLaunch) {
			...
			var curstate = {
				value: { game_progress: { correct:this.score,wrong:this.wrong } },
	...
	this.voiceCommandsEvent = function () {
		if (!fromLaunch) {
			...
			var curstate = { value:{ game_progress: {faster:this.faster,
				slower:this.slower,
	...
	this.deleteCategoryEvent = function (type) {
		if (!fromLaunch) {
			var time = moment.utc().format("x");
...
SorterGame.prototype.pickManyElements = function () {
	...
	for (var i = 0; i < newValue; i++) {
		...
		if (!this.unusedgameObjects.length) break //no gameover bug fix - gameOverModal appear
	...
	for (var i = l - 1; i >= 0; i--) {
		...
		if (myTarget.containsElement(element)) {
					...
					/*this.sortedObjectsEvent = {
						...
						state: {
							...
							game_progress: {
								...
								// event_count: this.sortedObjectsEvent.state.game_progress.event_count+1}
								//this inside object refers to...? maybe needs to change
							},
						},
					};
					sendEvent(this.sortedObjectsEvent, "Hitting a right target");
					*/
			...
			if (!correctness)	{
				...
				sendEvent(this.sortedObjectsEvent, "Hitting a wrong target");
				*/
